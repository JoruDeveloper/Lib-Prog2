...
// #include "node_bin_tree.h"
// #include <iostream>
// #include <list>
// #include <queue>
// #include <cstddef>   // For NULL
// #include <stdexcept> // For exceptions
// #include <vector>    // Needed for some path manipulation logic backups

// // Forward declaration can sometimes help in complex template scenarios
...
//     NodeBinTree<elem>* COPY_NODES(const NodeBinTree<elem>* sourceNode);
//     void DESTROY_NODES(NodeBinTree<elem>* node);
//     int COUNT_NODES(const NodeBinTree<elem>* node) const; // Helper to count nodes for subtree weight
...
//     // BST Helpers
//     void INSERT_BST(NodeBinTree<elem>* &node, const elem& value); // Pass node by ref to modify it
//     bool SEARCH_BST(const NodeBinTree<elem>* node, const elem& value) const;
//     NodeBinTree<elem>* FIND_MIN(NodeBinTree<elem>* node) const; // Finds minimum node in a subtree (for deletion)
//     NodeBinTree<elem>* REMOVE_BST(NodeBinTree<elem>* node, const elem& value, bool& removed); // Returns the modified subtree root
...
//     // Diameter Helpers (Mirroring Arbol_bin logic)
//     void COMBINE_PATHS(std::list<elem>& resultPath, std::list<elem> path1, std::list<elem> path2) const; // Like CAMINO_ENTRE_HOJAS
//     void UPDATE_LONGEST_PATH(std::list<elem>& currentLongest, std::list<elem>& path1, std::list<elem>& path2, const std::list<elem>& newBranch) const; // Like CAMINO_MAS_LARGO
//     void FIND_DIAMETER(NodeBinTree<elem>* node, std::list<elem>& currentLongestPath, std::list<elem>& longestBranch1, std::list<elem>& longestBranch2, std::list<elem>& currentBranch) const; // Like DIAMETRO
...
//     std::list<elem> getLevel(int level) const; // Arreglar
...
// template <class elem>
// NodeBinTree<elem>* BinTree<elem>::COPY_NODES(const NodeBinTree<elem>* sourceNode) {
...
//     // Note: Weight increment responsibility moved to public wrapper for clarity
...
//     if (it != inOrderList.end()) it++; // Skip root
...
//     // Recursive calls Right then Left
...
//         INSERT_BST(node->left, value); // Use getLeft()/setLeft() if NodeBinTree members were private
//     } else { // Insert duplicates to the right (or update if needed)
//         INSERT_BST(node->right, value); // Use getRight()/setRight() if NodeBinTree members were private
//     }
//     // Direct manipulation assumes `left` and `right` are accessible or via setters
//     // Corrected based on NodeBinTree having private members and public setters/getters:
//     if (node == NULL) {
//          node = new NodeBinTree<elem>(value);
//          this->weight++;
//     } else if (value < node->getInfo()) {
...
//         node->setLeft(leftChild); // Re-set in case the pointer changed (new node created)
//     } else {
//         NodeBinTree<elem>* rightChild = node->getRight();
//         INSERT_BST(rightChild, value);
//         node->setRight(rightChild); // Re-set in case pointer changed
//     }
...
//     } else if (value > node->getInfo()) { // Changed from >= to > to match Arbol_bin logic (search > goes right)
...
//             delete node;
//             return temp; // Return the right child (or NULL) to replace node
//         } else if (node->getRight() == NULL) {
//             NodeBinTree<elem>* temp = node->getLeft();
//             delete node;
//             return temp; // Return the left child to replace node
//         }
...
//         bool successorRemoved = false; // Dummy variable, removal success is handled by return value
//         node->setRight(REMOVE_BST(node->getRight(), temp->getInfo(), successorRemoved));
//     }
...
//     if (node == NULL || found) return; // Stop if null or already found
...
//     if (found) return; // Stop if found in left subtree

//     // Explore right
//     FIND_PATH(node->getRight(), currentPath, target, found);
//     if (found) return; // Stop if found in right subtree

//     // If not found down either path, backtrack
...
//      // Add current node to paths if corresponding target not yet found
...
//      // If both found, stop recursion
//      if (found1 && found2) return;

//      // Explore left subtree
//      FIND_PATHS_TO_TWO(node->getLeft(), path1, path2, target1, target2, found1, found2);
//      if (found1 && found2) return; // Stop if both found after left exploration

//      // Explore right subtree
//      FIND_PATHS_TO_TWO(node->getRight(), path1, path2, target1, target2, found1, found2);
//      if (found1 && found2) return; // Stop if both found after right exploration

//      // Backtrack: remove current node if corresponding target wasn't found below this path
...
//     resultPath.clear();
//     if (!lcaFound && !path1.empty()) lca = path1.front(); // Should ideally handle case where one path is empty
//     else if (!lcaFound && !path2.empty()) lca = path2.front();
//     // If lcaFound is still false, something is wrong (e.g., paths are empty or don't diverge from root)

//     // Reverse the remaining part of path1
//     std::vector<elem> reversedPath1;
//     while (!path1.empty()) {
//         reversedPath1.push_back(path1.front());
//         path1.pop_front();
//     }
//     // Add reversed path1 to result
//     // C++98 doesn't have vector reverse iterators easily accessible, manual reverse:
//      for (int i = reversedPath1.size() - 1; i >= 0; --i) {
//          resultPath.push_back(reversedPath1[i]);
//      }

//     // Add LCA
//     if (lcaFound) { // Only add LCA if it was part of the common prefix
//         resultPath.push_back(lca);
//     } else if (!resultPath.empty() && !path2.empty() && resultPath.back() == path2.front()) {
//          // Handle case where LCA is the start of path2 after path1 is exhausted
//          path2.pop_front(); // Remove duplicate LCA
//     } else if(lcaFound == false && !resultPath.empty() && path2.empty()){
//         // LCA was the end of path1, path2 was empty beyond LCA
//     } else if (lcaFound == false && resultPath.empty() && !path2.empty()){
//          // LCA was the start of path2, path1 was empty beyond LCA
//         resultPath.push_back(path2.front()); // Add LCA
//         path2.pop_front();
//     } // Complex edge cases for LCA being one of the nodes might need more checks
...
//      // Combine newBranch with path1 and path2 to form potential new diameters
//     std::list<elem> diameter1, diameter2;

//     if (!path1.empty()) {
//         COMBINE_PATHS(diameter1, path1, newBranch);
//     } else {
//         // If path1 is empty, the new branch is the first path found originating from the current subtree root
//         path1 = newBranch; // Update path1 to be this branch
//         return; // No diameter to compare yet
//     }

//     if (!path2.empty()) {
//         COMBINE_PATHS(diameter2, path2, newBranch);
//     } else {
//         // If path2 is empty, the new branch is the second path found
//         path2 = newBranch; // Update path2 to be this branch
//         // Now we have two branches, compare the diameter formed by path1 and path2
...
//     // Compare the two newly formed diameters and update the longest overall
//     if (diameter1.size() >= diameter2.size()) {
//         if (diameter1.size() > currentLongest.size()) {
//             currentLongest = diameter1;
//             // Update the branches that form this longest path if needed (complex)
//             // Arbol_bin logic updated branches here, perhaps to prune search later?
//              // path2 = newBranch; // Heuristic: keep the new branch as one side?
//         }
//     } else {
//         if (diameter2.size() > currentLongest.size()) {
//             currentLongest = diameter2;
//             // Update branches
//             // path1 = newBranch; // Heuristic?
//         }
//     }
//      // Decide which branch to keep/replace (path1 or path2) based on which one
//      // combined with newBranch created the longer path *locally*. This is heuristic.
//      // A simpler approach might be just comparing lengths.
//      // Let's stick to the core comparison: update currentLongest if a longer path is found.
//      // Updating path1/path2 based on local comparison helps subsequent calls in Arbol_bin.
//      // If diameter1 was longer or equal, maybe newBranch replaces path2?
...
//     if (node->getLeft() == NULL && node->getRight() == NULL) {
//         // Reached a leaf node, process this branch against existing longest branches
//         UPDATE_LONGEST_PATH(currentLongestPath, longestBranch1, longestBranch2, currentBranch);
//     } else {
//         // Recurse down, creating copies of branches for each side
//         std::list<elem> branch1_copy = longestBranch1;
//         std::list<elem> branch2_copy = longestBranch2;
//         FIND_DIAMETER(node->getLeft(), currentLongestPath, branch1_copy, branch2_copy, currentBranch);
//         // Update longest branches based on results from left subtree exploration
//         longestBranch1 = branch1_copy;
//         longestBranch2 = branch2_copy;

//         // Reset branch copies for right subtree exploration
//         branch1_copy = longestBranch1;
//         branch2_copy = longestBranch2;
//         FIND_DIAMETER(node->getRight(), currentLongestPath, branch1_copy, branch2_copy, currentBranch);
//          // Update longest branches based on results from right subtree exploration
//         longestBranch1 = branch1_copy;
//         longestBranch2 = branch2_copy;
//     }

//     currentBranch.pop_back(); // Backtrack
// }


// // --- Public Methods ---

// template <class elem> BinTree<elem>::BinTree() : root(NULL), weight(0) {}
// template <class elem> BinTree<elem>::BinTree(elem e) : root(new NodeBinTree<elem>(e)), weight(1) {}
// template <class elem> BinTree<elem>::BinTree(const BinTree<elem>& otherTree) : root(NULL), weight(0) { *this = otherTree; } // Use assignment op
// template <class elem> BinTree<elem>::BinTree(elem e, const BinTree<elem>& leftTree, const BinTree<elem>& rightTree) {
//     root = new NodeBinTree<elem>(e);
//     root->setLeft(COPY_NODES(leftTree.root));
//     root->setRight(COPY_NODES(rightTree.root));
//     // Calculate weight accurately
//     weight = 1 + COUNT_NODES(root->getLeft()) + COUNT_NODES(root->getRight());
// }
// template <class elem> BinTree<elem>::~BinTree() { DESTROY_NODES(root); }

// template <class elem>
// BinTree<elem>& BinTree<elem>::operator=(const BinTree<elem>& otherTree) {
//     if (this != &otherTree) {
//         CLEAR();
//         root = COPY_NODES(otherTree.root);
//         weight = otherTree.weight; // Direct copy of weight is fine after deep node copy
//     }
//     return *this;
// }
...
//          CLEAR();
//      } else if (this != otherTreePtr) { // Check self-copy via pointer
//         CLEAR();
//         root = COPY_NODES(otherTreePtr->root);
//         weight = otherTreePtr->weight;
//      }
// }

// template <class elem> bool BinTree<elem>::isEmpty() const { return root == NULL; }
// template <class elem> int BinTree<elem>::getWeight() const { return weight; }
// template <class elem> elem BinTree<elem>::getRootInfo() const {
...
//         leftSubtree.root = COPY_NODES(root->getLeft());
//         leftSubtree.weight = leftSubtree.COUNT_NODES(leftSubtree.root); // Use helper
//     }
//     return leftSubtree;
// }
...
//         rightSubtree.root = COPY_NODES(root->getRight());
//         rightSubtree.weight = rightSubtree.COUNT_NODES(rightSubtree.root); // Use helper
//     }
//     return rightSubtree;
// }

// template <class elem> std::list<elem> BinTree<elem>::preOrder() const { std::list<elem> r; PRE_ORDER(root, r); return r; }
// template <class elem> std::list<elem> BinTree<elem>::inOrder() const { std::list<elem> r; IN_ORDER(root, r); return r; }
// template <class elem> std::list<elem> BinTree<elem>::postOrder() const { std::list<elem> r; POST_ORDER(root, r); return r; }
// template <class elem> std::list<elem> BinTree<elem>::levelOrder() const { std::list<elem> r; LEVEL_ORDER_HELPER(root, r); return r; }
...
//             currentLevel++;
//             if (q.empty() || currentLevel > level) break; // Stop if queue empty or past target level
...
// template <class elem> int BinTree<elem>::getHeight() const { int ml = -1; GET_HEIGHT(root, 0, ml); return ml; }
// template <class elem> std::list<elem> BinTree<elem>::getLeaves() const { std::list<elem> r; GET_LEAVES(root, r); return r; }

// template <class elem>
// void BinTree<elem>::insertBST(const elem& value) {
//     // The helper INSERT_BST passes the node pointer by reference,
//     // so it directly modifies the tree structure (root or children links).
//     INSERT_BST(this->root, value);
//     // Weight is incremented inside the helper when a new node is created.
// }
...
//      // Weight is decremented inside REMOVE_BST when node is confirmed for removal
//      return removed;
// }
...
//     CLEAR();
//     root = BUILD_FROM_PRE_IN_RECURSIVE(preOrderList, inOrderList);
//     // Re-calculate weight after building (helper modified lists, can't reliably track there)
//     weight = COUNT_NODES(root);
// }
...
//     CLEAR();
//     root = BUILD_FROM_POST_IN_RECURSIVE(postOrderList, inOrderList);
//     // Re-calculate weight after building
//     weight = COUNT_NODES(root);
// }
...
//     std::list<elem> path;
//     bool found = false;
//     FIND_PATH(root, path, target, found);
//     if (!found) path.clear(); // Return empty list if not found
//     return path;
// }
...
//     FIND_PATHS_TO_TWO(root, path1, path2, value1, value2, found1, found2);

//     if (found1 && found2) {
//         COMBINE_PATHS(resultPath, path1, path2);
//     }
//     // else: one or both nodes not found, return empty list
...
//     if (!lcaFound) {
//          // Should not happen if root exists and both found, unless tree is empty/root is null
//          if(isEmpty()) throw std::runtime_error("Cannot find LCA in an empty tree");
//          else throw std::runtime_error("Could not determine LCA (logic error or paths incorrect)"); // Should technically find root at least
//     }
...
//     if (isEmpty()) return longestPath; // Empty path for empty tree

//     std::list<elem> branch1, branch2, currentBranch;
//     FIND_DIAMETER(root, longestPath, branch1, branch2, currentBranch);

//     // The longest path might be a single branch from root if tree is skewed
...
void BinTree<elem>::CLEAR() {
    DESTROY_NODES(root);
    this->root = NULL; // Use this-> for clarity within member function
    this->weight = 0;
}
...
template <class elem>
void BinTree<elem>::INSERT_BST(NodeBinTree<elem>* &node, const elem& value) {
    // Cleaned up version - Uses getters/setters only
    if (node == NULL) {
         node = new NodeBinTree<elem>(value);
         this->weight++;
    } else if (value < node->getInfo()) {
...
BinTree<elem>::BinTree(elem e) {
    // Definition style changed (though only one line originally)
...
BinTree<elem>::BinTree(const BinTree<elem>& otherTree) {
    // Use assignment operator for implementation consistency
    this->root = NULL; // Initialize before assignment
    this->weight = 0;
    *this = otherTree;
}
